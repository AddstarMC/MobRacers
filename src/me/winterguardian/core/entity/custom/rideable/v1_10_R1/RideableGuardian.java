...
public class RideableGuardian extends EntityGuardian implements RideableEntity {
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.noclip) {
            ...
            a(d1, this.onGround, iblockdata, blockposition);
            if (d6 != d0) {
                this.motX = 0.0D;
            }
            if (d8 != d2) {
            ...
            if ((playStepSound()) && (!flag) && (!isPassenger()))
            {
                ...
                if ((this.K > EntityUtil.getProtectedField("ax",this, Entity.class, int.class)) && (iblockdata.getMaterial() != Material.AIR)) {
                    ...
                    if (isInWater()) {
                        ...
                        a(aa(), f, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.4F);
                    }
                    a(blockposition, block1);
                }
            }
            boolean flag2 = this.ai();
            if (this.world.f(getBoundingBox().shrink(0.001D))) {
                burn(1.0F);
                if (!flag2) {
                    this.fireTicks += 1;
                    if (this.fireTicks <= 0) {
            ...
            if ((flag2) && (this.fireTicks > 0)) {
                a(SoundEffects.bG, 0.7F, 1.6F + (this.random.nextFloat() - this.random.nextFloat()) * 0.4F);
                this.fireTicks = (-this.maxFireTicks);
            }
            this.world.methodProfiler.b();
        }
    ...
    @Override //Ambient Sound effect
    protected SoundEffect G() {
        if (isElder()) return SoundEffects.aD;
        return SoundEffects.cg;
    }

    @Override //returns hurt sound effect
    protected SoundEffect bV() {
        if (isElder()) return SoundEffects.aI;
