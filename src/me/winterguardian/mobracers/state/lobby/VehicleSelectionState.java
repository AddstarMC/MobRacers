...
public class VehicleSelectionState extends WaitingState implements VehicleState
{
	...
	public void join(Player p)
	{
		...
		if(getGame().getConfig().isColorInTab())
			p.setPlayerListName(ChatColor.DARK_GREEN.toString() + p.getName());
	...
	public void onChoose(final Player p, VehicleType type, boolean force)
	{
		...
		if(!((MobRacersConfig) getGame().getConfig()).enableStats() || p.hasPermission(MobRacersPlugin.ALL_UNLOCKED) || vehicle.canChoose(p)) 
		{
			...
			p.sendMessage(CourseMessage.SEPARATOR_SELECTVEHICLE.getPrefix() + ChatColor.WHITE.toString() + ChatColor.BOLD.toString() + vehicle.getName());
			p.sendMessage(ChatColor.YELLOW.toString() + ChatColor.BOLD.toString() + vehicle.getDescription());
			p.sendMessage(" ");
		...
		if(vehicle instanceof WinnableVehicle)
		{
			...
			p.sendMessage(achievement.getName() + ChatColor.RESET.toString() +"- " + achievement.getDescription() + ChatColor.WHITE.toString()  + achievement.getProgression(p));
		}
