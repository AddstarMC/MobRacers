...
public class ArenaSubCommand extends SubCommand
{
	...
	{
		super("arena", Arrays.asList("arene", "ar√®ne", "mapconfig"), MobRacersPlugin.STAFF, CourseMessage.COMMAND_INVALID_PERMISSION.toString(), "ßc"+ CourseMessage.COMMAND_USAGE + ": ßf/mobracers arena help");
	}
	...
	public boolean onSubCommand(final CommandSender sender, String label, String[] args)
	{
		...
		if(args[0].equalsIgnoreCase("list") || args[0].equalsIgnoreCase("liste"))
		{
			if(Arena.getArenaList().size() > 0)
			{
				...
				for(Arena arena : Arena.getArenaList())
					sender.sendMessage("  ßf>" + (arena.isReady() ? "ßa" : "ßc") + arena.getName());
			}
			else
		...
		if(args[0].equalsIgnoreCase("new") || args[0].equalsIgnoreCase("nouveau") || args[0].equalsIgnoreCase("nouvelle") || args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("cr√©er"))
		{
			if(args.length < 2)
			{
				CourseMessage.COMMAND_INVALID_ARGUMENT.say(sender);
		...
		{
			CourseMessage.COMMAND_ARENA_INFO_HEAD.say(sender, "<arena>", (arena.isReady() ? "ßa" : "ßc") + arena.getName());
			CourseMessage.COMMAND_ARENA_INFO_BREAKLINES.say(sender, "#", "" + arena.getLines().size());
			CourseMessage.COMMAND_ARENA_INFO_WEATHER.say(sender, "<time>", "" + arena.getTime(), "<time-locked>", arena.isTimeLocked() ? "ßatrue" : "ßcfalse", "<raining>", arena.isRaining() ? "ßatrue" : "ßcfalse");
			...
			CourseMessage.COMMAND_ARENA_INFO_REGION.say(sender, "<region>", arena.isRegionReady() ? "ßatrue" : "ßcfalse");
			...
			if(arena.getRoadSpeedModifier().size() != 0)
			{
				for(RoadType road : arena.getRoadSpeedModifier().keySet())
					sender.sendMessage("  ße" + road.toString() + " ßf-> " + (arena.getRoadSpeedModifier().get(road) > 0 ? "ßa+" : "ßc") + arena.getRoadSpeedModifier().get(road));
			}
			else
		...
		if(args[0].equalsIgnoreCase("setregion") || args[0].equalsIgnoreCase("r√©gion"))
		{
		...
		if(args[0].equalsIgnoreCase("weather") || args[0].equalsIgnoreCase("m√©t√©o") || args[0].equalsIgnoreCase("meteo"))
		{
		...
		if(args[0].equalsIgnoreCase("setitemregendelay") || args[0].equalsIgnoreCase("d√©laiitems") || args[0].equalsIgnoreCase("setregendelay"))
		{
			if(args.length < 3)
			{
				CourseMessage.COMMAND_INVALID_ARGUMENT.say(sender);
					...
					Bukkit.getScheduler().runTaskLater(MobRacersPlugin.getPlugin(), new Runnable()
					{
						...
						public void run()
						{
							if(((Player) sender).isOnline())
							{
								...
								ActionBarUtil.sendActionMessage((Player) sender, "ßeLine #" + j);
							}
						}
					}, i * 20);
				}
		...
		if(args[0].equalsIgnoreCase("resetlines") || args[0].equalsIgnoreCase("r√©initlignes"))
		{
		...
		if(args[0].equalsIgnoreCase("addspawn") || args[0].equalsIgnoreCase("ajoutd√©part"))
		{
			if(!(sender instanceof Player))
			{
				CourseMessage.COMMAND_INVALID_SENDER.say(sender);
		...
		if(args[0].equalsIgnoreCase("removespawn") || args[0].equalsIgnoreCase("retraitd√©part") || args[0].equalsIgnoreCase("delspawn"))
		{
			if(!(sender instanceof Player))
			{
				CourseMessage.COMMAND_INVALID_SENDER.say(sender);
		...
		if(args[0].equalsIgnoreCase("resetspawns") || args[0].equalsIgnoreCase("r√©initd√©parts"))
		{
		...
		if(args[0].equalsIgnoreCase("showspawns") || args[0].equalsIgnoreCase("voird√©parts"))
		{
			for(SerializableLocation loc : arena.getSpawnPoints())
		...
		if(args[0].equalsIgnoreCase("resetjukeboxes") || args[0].equalsIgnoreCase("r√©initjukeboxes"))
		{
		...
		if(args[0].equalsIgnoreCase("resetspectators") || args[0].equalsIgnoreCase("r√©initspectateurs"))
		{
		...
		if(args[0].equalsIgnoreCase("resetitems") || args[0].equalsIgnoreCase("r√©inititems"))
		{
		...
		if(args[0].equalsIgnoreCase("resetdeathzones") || args[0].equalsIgnoreCase("r√©initzonesmortes"))
		{
