...
public class CoursePurchase
{
	...
	public String getPresentation(int points)
	{
		if(this.price <= points)
			return JsonUtil.toJson(this.getType().getName() + ChatColor.RESET + this.getName() + ChatColor.RESET.toString() + ChatColor.BOLD.toString() +
					"-" + ChatColor.GREEN.toString() + ChatColor.BOLD.toString() + CourseMessage.PURCHASE_PRICE.toString().replace("<price>", TextUtil.toString(this.getPrice())), "show_text", JsonUtil.toJson(CourseMessage.PURCHASE_CLICKTOBUY.toString()), "suggest_command", "\"/mobracers buy " + this.getName() + "\"");
